"""Pembagi otomatis 40 mahasiswa ke kelompok UAS Literasi Big Data PAI.
...
Cara pakai:
    python peserta/buat_kelompok.py            # pakai default seed
    python peserta/buat_kelompok.py --seed 7   # ulangi dengan seed lain
...
def assign_kelompok(peserta: List[Dict], seed: int) -> List[Dict]:
    ...
    for k_idx, ukuran in enumerate(UKURAN_KELOMPOK, start=1):
        ...
        # round-robin topik berdasarkan urutan kelompok
        topik = TOPIK_UAS[(k_idx - 1) % len(TOPIK_UAS)]
        for slot in range(ukuran):
